...
| `panic: ieee80211_get_ratetable: no rate table for channel; freq 0 flags 0x0` (at attach, right after `D11 LCN ucode uploaded`) | The driver attached net80211 before populating its channel list, so `ieee80211_chan_init()` read a zeroed `ic_channels[0]` (freq 0 / flags 0x0). The `KASSERT` guards that catch this are compiled out on release kernels. Fixed in current code: `bcm4313_add_channels()` fills `ic_channels`/`ic_nchans` before `ieee80211_ifattach()` (the same order `bwn` uses). | `git pull` and rebuild. |
