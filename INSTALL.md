...
| `bcm43130: unsupported PHY type 15 (rev 15)` | Reading `PHYVER` as `0xFFFF` — the core was brought out of reset without the PHY clock enabled, so PHY registers read as garbage (type 15 = all-ones). Fixed in current code: attach now resets with `PHYCLOCK_ENABLE \| PHYRESET \| SUPPORT_G` (bwn's `bwn_reset_core` pattern) before probing. | `git pull` and rebuild. |
